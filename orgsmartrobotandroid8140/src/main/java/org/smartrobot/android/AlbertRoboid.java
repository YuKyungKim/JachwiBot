...
final class AlbertRoboid extends PhysicalRoboid
{
	...
	private final byte[] mMotoringSimulacrum = new byte[988];
	private final int[][] mReadBuffer = new int[21][];
	private final int[][] mWriteBuffer = new int[11][];
	...
	byte[] encodeMotoringSimulacrum()
	{
		synchronized(mWriteLock)
		{
			...
			n3(mMotoringSimulacrum, mWriteMotoringFlag, buffer[0], buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10]);
