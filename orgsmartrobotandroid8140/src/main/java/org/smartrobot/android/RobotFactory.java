...
{
	static AbstractRobot create(String id)
	{
		if(Alpha.ID.equals(id))
			return new AlphaRobot();
		else if(Albert.ID.equals(id))
			return new AlbertRobot();
		else if(AlbertPop.ID.equals(id))
			return new AlbertPopRobot();
		return null;
	}
}