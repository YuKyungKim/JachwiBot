...
/**
 * <p>알버트 로봇의 모델 ID와 알버트 로봇을 구성하는 각 디바이스의 ID에 대한 상수 값을 정의한다.
 * <p>알버트 로봇은 8개의 센서 디바이스와 9개의 이펙터 디바이스, 2개의 커맨드 디바이스, 2개의 이벤트 디바이스로 구성되어 있으며, 각 디바이스의 데이터는 정수형 배열로 선언되어 있다.
 * <p>알버트 로봇을 구성하는 디바이스의 데이터를 읽거나 쓰기 위해서는 우선 {@link org.roboid.robot.Robot Robot} 클래스의 {@link org.roboid.robot.Robot#findDeviceById(int) findDeviceById(int deviceId)} 메소드를 사용하여 디바이스 객체의 레퍼런스를 얻어야 한다.
 ...
 * void someMethod(Robot robot)
 {
     Device deviceLip = robot.findDeviceById(Albert.EFFECTOR_LIP); // 입 이펙터 디바이스를 얻는다.
     int lip = deviceLip.read(); // 입 크기 값을 읽는다.

     Device deviceLeftProximity = robot.findDeviceById(Albert.SENSOR_LEFT_PROXIMITY); // 왼쪽 근접 센서 디바이스를 얻는다.
     int[] leftProximity = new int[4];
     deviceLeftProximity.read(leftProximity); // 왼쪽 근접 센서 값을 읽는다.

     Device devicePosition = robot.findDeviceById(Albert.SENSOR_POSITION); // 위치 센서 디바이스를 얻는다.
     int positionX = devicePosition.read(0); // 위치 데이터의 X 좌표 값을 읽는다.
     int positionY = devicePosition.read(1); // 위치 데이터의 Y 좌표 값을 읽는다.

     Device deviceFrontOID = robot.findDeviceById(Albert.EVENT_FRONT_OID); // 앞쪽 OID 이벤트 디바이스를 얻는다.
     if(deviceFrontOID.e()) // 앞쪽 OID 값이 갱신되었는지 확인한다.
     {
         int frontOID = deviceFrontOID.read(); // 앞쪽 OID 값을 읽는다.
     }

     Device deviceLeftWheel = robot.findDeviceById(Albert.EFFECTOR_LEFT_WHEEL); // 왼쪽 바퀴 이펙터 디바이스를 얻는다.
     deviceLeftWheel.write(25); // 왼쪽 바퀴 속도 값을 쓴다.

     Device deviceLeftEye = robot.findDeviceById(Albert.EFFECTOR_LEFT_EYE); // 왼쪽 눈 이펙터 디바이스를 얻는다.
     int[] leftEye = new int[] { 255, 0, 0 };
     deviceLeftEye.write(leftEye); // 왼쪽 눈 색상 값을 쓴다.

     Device deviceFrontLED = robot.findDeviceById(Albert.COMMAND_FRONT_LED); // 앞쪽 LED 커맨드 디바이스를 얻는다.
     deviceFrontLED.write(1); // 앞쪽 LED를 켠다.

     Device deviceBodyLED = robot.findDeviceById(Albert.EFFECTOR_BODY_LED); // 몸통 LED 이펙터 디바이스를 얻는다.
     deviceBodyLED.write(50); // 몸통 LED의 밝기 값을 쓴다.

     Device devicePadSize = robot.findDeviceById(Albert.COMMAND_PAD_SIZE); // 패드 크기 커맨드 디바이스를 얻는다.
 ...
 public void onDeviceDataChanged(Device device, Object values, long timestamp)
 {
     ...
     switch(device.getId())
     {
     case Albert.EFFECTOR_LEFT_WHEEL: // 왼쪽 바퀴 속도 값을 얻는다.
         leftWheel = ((int[])values)[0];
         break;
     case Albert.COMMAND_PAD_SIZE: // 패드 크기 값을 얻는다.
         padWidth = ((int[])values)[0];
         padHeight = ((int[])values)[1];
         break;
     case Albert.SENSOR_LEFT_PROXIMITY: // 왼쪽 근접 센서 값을 얻는다.
         System.arraycopy((int[])values, 0, leftProximity, 0, 4);
         break;
     case Albert.EVENT_FRONT_OID: // 앞쪽 OID 값을 얻는다.
...
{
	/**
	 * <p>알버트 로봇의 모델 ID를 나타내는 상수.
	 * </p>
	 * <ul>
	 *     <li>상수 값: "kr.robomation.physical.albert"
	 * </ul>
	 */
	 ...
	 * 알버트 로봇의 입은 실제 하드웨어 장치에는 없는 가상의 장치이며, 이 값을 이용하여 화면에 그래픽을 표시하거나 LED를 깜박이도록 하여 입을 움직이는 것과 비슷한 효과를 나타낼 수 있다.
	...
	/**
	 * <p>몸통 LED 이펙터 디바이스의 ID를 나타내는 상수.
	 * <p>몸통 LED 이펙터 디바이스의 데이터는 몸통 LED의 밝기를 나타낸다.
	 * 값이 클수록 밝아진다.
	 * </p>
	 * <ul>
	 *     <li>상수 값: 0x00200013
